...
export default function CreateClients() {
  ...
  const [state, setstate] = React.useState(() => {
    if (!id) {
      // Yangi yaratish uchun har doim 1 dan boshlash
      return 1;
    }
    // Edit qilish uchun localStorage dan olish
      ...
      mutation.mutate(formData, {
        onSuccess: () => {
          ...
          toast.success("ok create");
           localStorage.removeItem("client_form_state")
        },
        onError: (error) => {
  ...
  const handleStateChange = (newState) => {
    setstate(newState);
    localStorage.setItem("client_form_state", newState.toString());
};
...
useEffect(() => {
    if (id) {
      ...
      // Create mode - formlarni tozalash
      reset({});
      resetsecond({});
    }
  }, [DD, result, reset, resetsecond, state, id]);
  return (
    <div>
      {isLoading || load ? (
        <div>LOADING...</div>
