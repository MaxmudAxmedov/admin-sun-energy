...
export default function EmployeesCrud() {
  ...
  const mutations = useMutation(postemployeeMutation);
  ...
  const Submit = (data) => {
    ...
    if(data.filed && data.file.size>0){
    ...
    <div className='pl-[35px]'>
      ...
      <form onSubmit={handleSubmit(Submit)}>
        <div className="flex px-5 py-4 max-w-[800px] justify-between items-center gap-4 flex-wrap">
          ...
          <label className="text-active" htmlFor="name">
            {t("phone")}
            <Input {...register("phone")} value={phone}
              onChange={(e) => setPhone(e.target.value)} className="max-w-[400px] transition-all focus:w-64 outline-none text-active" placeholder={t("phone")} />
          </label>
        ...
        <div className="ml-5">
          <ImageUploadForm register={register} />
        </div>
